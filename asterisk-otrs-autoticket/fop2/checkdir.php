...
//echo window.open('http://suporte.synapsebrasil.com.br/otrs/index.pl);
...
//echo "window.open('/vtigercrm/index.php?module=Home&action=UnifiedSearch&query_string=$clidnum')";
        ...
        $uniqueid = $ALLVAR['UNIQUEID'];

        $dbGetCmd = $ASTERISK_CLI_CMD . "\"database get $ASTERISK_DEFAULT_DB_FAMILY $uniqueid\"";
        $dbDelCmd = $ASTERISK_CLI_CMD . "\"database del $ASTERISK_DEFAULT_DB_FAMILY $uniqueid\"";
...
if ($largo > 5) { // prevent short numbers (extensions) from trigger a lookup
